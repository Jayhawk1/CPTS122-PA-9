...
int main() {
  ...
  EntityList<Entity> Entities;
    ...
    WalkerEnemy enemy2(BLUE, { 300, 250 });
    WalkerEnemy enemy3(BLUE, { 300, 450 });
    WalkerEnemy enemy4(BLUE, { 300, 650 });
    ...
    */
    ...
    while (window.isOpen()) {
        ...
        if (state == GameState::Playing) {
			...
			std::vector<Pickup> result = CurRoom.getPickups();
            for (Pickup val : result) {
                player.update(val);
            }
